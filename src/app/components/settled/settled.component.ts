...
export class SettledComponent implements OnInit {
  ...
  constructor(private precedentsservice:PrecedentsService,private settledService:SettledService,private fb: FormBuilder) {
    this.formSettled = fb.group({
      ...
      corte: new FormControl('', [Validators.required,Validators.pattern(/^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/)]),
  ...
  ngOnInit(): void {
    ...
    this.allInstitucion = this.settledService.getInstitucion();
  }
